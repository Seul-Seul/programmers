...
자릿수가 주어지지 않아 문자열로 만든 후 한글자씩 잘라 배열에 저장 후
배열에서 하나씩 꺼내 정수형으로 변환 후 더해주었다

자릿수를 입력 받았다면 자릿수만큼 나머지연산을 해서 하나씩 더할려 했다. 
내가 푼 방법은 타입 변환을 두번 했다는 점에서 효율성이 떨어져 보인다.

다른 사람의 풀이를 보니 자릿수를 알 필요가 없었다!.. while문돌리면 끝인걸 ㅋ 
