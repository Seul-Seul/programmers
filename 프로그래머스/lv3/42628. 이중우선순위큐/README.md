...
---
### 후기
        최댓값과 최솟값을 구하기 위해 그냥 Queue대신 우선순위 큐를 사용했다. 
        처음에 우선순위 큐 하나만 만들었더니 명령어에 따라 최댓값과 최솟값 중 하나는 버려지는 일이 발생했다.(당연하지이눔아) 그래서 큐를 두개 만들어서 따로 관리해줌
        우선순위 큐 클래스에 인자로 comparator.reverseOrder()를 인자로 주면 내림차순 안주면 오름차순으로 데이터를 정리한다.
        최댓값큐와 최솟값큐를 각각 만든 후 명령어를 실행할 때마다 두가지 큐 모두에 적용하고 마지막 값을 각각 빼내면 최댓값과 최솟값 완성~!
